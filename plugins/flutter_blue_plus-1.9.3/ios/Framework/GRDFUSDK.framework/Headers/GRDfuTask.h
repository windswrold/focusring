...
typedef NS_OPTIONS(NSUInteger, GRDTaskType) {
    ...
    GRDTaskTypeFASTDFU  = 1 << 10,// 1024, FASTDFU,固件升级
    GRDTaskTypeFASTDFUResource  = 1 << 11,// 2048, FASTDFU资源升级
};
...
-(void)dfuTaskDidUpdateProgress:(float)percent type:(GRDTaskType)type extraInfo:(NSDictionary*)extraInfo progressType:(int)progressType;
...
- (instancetype)attachWithIdentifier:(NSUUID *)identifier centralManager:(CBCentralManager *)mgr setGusMode:(GRDFirmware*)fw isCopyMode : (BOOL)isCopyMode copyAddr : (uint32_t) copyAddr;

- (instancetype)attachWithIdentifier:(NSUUID *)identifier centralManager:(CBCentralManager *)mgr resourceData:(NSData*)data toExtFlash : (BOOL)toExtFlash toAddr:(uint32_t)toAddr;
